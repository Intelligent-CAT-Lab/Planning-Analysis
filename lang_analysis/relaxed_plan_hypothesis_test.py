...
CORE LOGIC (RELAXED VERSION):
1. Presence check: All expected phases must appear in trajectory (with prefix matching)
2. Order check: First appearances must follow strictly increasing order

RELAXATION:
- Extra phases (not in expected phases) are ALLOWED and IGNORED for plan-checking
- Only expected phases are validated for presence and order
...
USAGE:
  ...
  python relaxed_plan_hypothesis_test.py --agent SWE-agent --setting plan --model claude-sonnet-4

  # Multiple models test with setting
  python relaxed_plan_hypothesis_test.py --agent SWE-agent --setting plan --models model1 model2 model3
...
def check_trajectory_compliance(
    ...
    RELAXED VERSION: The trajectory is compliant if:
    1. All expected phases appear in the trajectory (with prefix matching)
    2. The first appearance indices follow a strictly increasing order

    RELAXATION: Extra phases (not in expected phases) are ALLOWED and IGNORED.
    Only expected phases are validated for presence and order.
    ...
    Example 2 (Compliant - extra phases allowed):
        languatory = ["L_navigate_2", "L_reproduce_2", "X_unknown_1", "P_1"]
        expected_order = ("L_navigate", "L_reproduce", "P")

        X_unknown is ignored (extra phase allowed) ✓
        Expected phases present with correct order ✓
        Returns True
    ...
    Example 4 (Non-compliant - wrong order):
        languatory = ["L_reproduce_2", "L_navigate_1", "P_1"]
        expected_order = ("L_navigate", "L_reproduce", "P")

        All phases present but wrong order ✗
        Returns False
    """
    first_appearances = extract_first_appearances(languatory)

    # Step 1: Build mapping from expected phases to their actual first appearances
    # using prefix matching (RELAXED: no alphabet check, extra phases ignored)
    expected_to_actual = {}
    ...
    # Step 2: Check if the first appearances are in strictly increasing order
...
def run_single_model_test(config: PlanTestConfig, model: str) -> None:
    ...
    output_path = Path(config.data_dir) / config.agent / config.setting / model / "stats" / "plan_hypothesis_test" / f"relaxed_{config.agent}_{config.setting}_{model}_test.txt"
