...
Phases before first patch ("localization / reproduction"):
  - L_reproduce_newly_generated_test : generating / viewing / executing tests (reproducing / understanding bug) that did NOT
  ...
  - L_reproduce_regression_test : generating / viewing / executing tests that DID
        originally exist in the repo, including running pytest or editing
        existing tests.
...
Phases after first patch ("validation"):
  - V_newly_generated_test :
        ...
        originally exist in the repo.
        Repeats of those same new tests (including the ones generated in L_repeoduce_newly_generated_test) are still newly_generated.
  - V_regression_test :
...
# --------------------------- Test classification (pre/post-patch) ---------------------------

def _classify_test_interaction(
    targets: List[str],
    *,
    created_tests: Optional[Set[str]],
    ...
    """
    Classify test interaction as newly_generated vs regression.
    Works for both pre-patch (L_reproduce_*) and post-patch (V_*) phases.

    phase_prefix: either "L_reproduce" or "V"
    """
    if targets:
        if created_tests:
            ...
            for p in targets:
                ...
                if p in created_tests:
                    return f"{phase_prefix}_newly_generated_test"
                ...
                for created_path in created_tests:
                    ...
                    if p_basename == created_basename:
                        return f"{phase_prefix}_newly_generated_test"
        return f"{phase_prefix}_regression_test"
...
def get_action_role(
    ...
    *,
    ...
    Classify a step into:
      "L_reproduce_newly_generated_test", "L_reproduce_regression_test",
      "L_navigate", "P",
    ...
    if cmd in PY_CMDS:
        ...
        if edited_files_from_code:
            ...
            if test_files_edited:
                # NOTE: We do NOT call _record_created_tests here because we cannot
                # distinguish between creating a new file vs editing an existing one
                # from code content alone. Files are only recorded as created when
                # we see explicit creation commands (e.g., SRE create, shell redirection).
                phase = "V" if has_patch else "L_reproduce"
                return _classify_test_interaction(
                    test_files_edited,
                    created_tests=created_tests,
        ...
        if has_patch:
            ...
            # Pre-patch: classify test interactions or return general
            if is_heredoc or test_related or cmd == "pytest" or dyn_key:
                return _classify_test_interaction(
                    explicit_test_targets,
                    created_tests=created_tests,
                    dynamic_key=dyn_key,
                    created_dynamic_suites=created_dynamic_suites,
                    phase_prefix="L_reproduce",
                )
            return "general"
