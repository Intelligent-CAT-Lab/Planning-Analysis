...
class GraphVisualizer:
    ...
    @staticmethod
    def _html_worker(G: nx.MultiDiGraph, html_path: str, template_dir: Path):
        """Worker process for HTML generation."""
        gv = GraphVisualizer(template_dir=template_dir)
        gv.draw_graph_html(G, html_path)

    @classmethod
    def draw_with_timeout(cls, G: nx.MultiDiGraph, html_path: str, 
                         timeout_sec: int = 100, template_dir: Path = None) -> bool:
    ...
    def _prepare_nodes_data(self, G: nx.MultiDiGraph) -> List[Dict[str, Any]]:
        ...
        for node_id, node_data in G.nodes(data=True):
            ...
            nodes_data.append({
                ...
                "color": primary_color,
                "colors": colors,
                "thought_max": max(thought_lengths) if thought_lengths else 0
            })
    ...
    def _prepare_edges_data(self, G: nx.MultiDiGraph) -> List[Dict[str, Any]]:
        ...
        for u, v, k, d in G.edges(keys=True, data=True):
            ...
            edges_data.append({
                ...
                "to": v,
                "type": etype,
                "label": str(d.get("label", "")) if etype == "exec" else ""
            })
        return edges_data

    def draw_graph_html(self, G: nx.MultiDiGraph, html_path: str):
